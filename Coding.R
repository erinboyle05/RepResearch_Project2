...
# storm_data$CROPDMGEXP <- gsub("", "1", storm_data$CROPDMGEXP)
...
q2_data <- fstorm_data %>%
        ...
        group_by(EVTYPE) %>%
        summarise(PROPDMG = mean(PROPDMG), CROPDMG = mean(CROPDMG), Total = mean(PROPDMG + CROPDMG)) %>%
        top_n(10, Total) %>%
        select(-Total) %>%
        melt(id.var = "EVTYPE")

q2_plot <- ggplot(q2_data, aes(x = EVTYPE, y = value, fill = variable)) +
        geom_bar(stat = "identity") +
        theme(axis.text.x = element_text(angle = 90, hjust = 1))
